...
automataValido(A) :- estadosNoFinalesSonSalientes(A), todosAlcanzables(A), hayEstadoFinal(A), noHayEstadosFinalesRepetidos(A), noHayTransicionesRepetidas(A).
%Evalúa todas los predicados presentados con A.

%estadosNoFinalesSonSalientes(+A)
estadosNoFinalesSonSalientes(A) :- estados(A,E), finalesDe(A,F), subtract(E,F,EstadosNoFinales), forall(member(X,EstadosNoFinales), caminoDeLongitud(A,2,_,_,X,_)), !.
...
palabraMasCorta(A, P) :- minimaLongitudAceptada(A,Len), inicialDe(A,Init), finalesDe(A,Finales), caminoDeLongitud(A, Len, _, P, Init, Fin), member(Fin,Finales).
%		Chequeo cual es la minima longitud de palabra que acepta el automata (con el predicado anterior), y chequeo
%		cuales son las listas de etiquetas P que son reconocidas con esa longitud.

%minimaLongitudAceptada(+A,-L)
minimaLongitudAceptada(A, N) :- inicialDe(A,Init), finalesDe(A,Finales), desde(1,N), caminoDeLongitud(A, N, _, _, Init, Fin), member(Fin, Finales), !.
